...
# read_arrow comes from the community nanoarrow extension (the core arrow extension was
# discontinued in DuckDB 1.3), and decoding geometry needs spatial. Installing an extension
# already present costs nothing, which keeps a local run self-sufficient.
require_duckdb() {
    command -v duckdb >/dev/null \
        || fail "duckdb is not on PATH; install it from https://duckdb.org/docs/installation/"
    duckdb -c "install nanoarrow from community; install spatial" >/dev/null 2>&1 \
        || fail "could not install the DuckDB nanoarrow and spatial extensions"
}
